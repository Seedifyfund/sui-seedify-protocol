...
const Claim: React.FC = () => {
  ...
  const fetchVestingStatus = async () => {
    try {
        ...
        filteredObjects.map(async (obj: any) => {
          try {
            ...
            return {
              ...
              claim_interval:
                Number(fields.claim_interval) / (24 * 60 * 60 * 1000),
              ...
              immediate_transfer_balance:
                Number(fields.immediate_transfer_balance) / 1_000_000_000,
      ...
      setWallets(
        fetchedWallets.filter((wallet) => wallet !== null) as WalletFields[]
      );
    } catch (error) {
  ...
  const claimImmediateTransfer = async (
    walletId: string,
    walletType: string
  ) => {
    try {
                    ...
                    <Table className="min-w-full text-white">
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead>Renouncement Start</TableHead>{" "}
                          {/* New Column */}
                          <TableHead>Renouncement End</TableHead>{" "}
                          {/* New Column */}
                      ...
                      <TableBody>
                        {wallets.map((wallet, index) => {
                          ...
                          const isImmediateClaimable =
                            !wallet.immediate_transfer_claimed &&
                            Date.now() >= wallet.immediate_claim_start;
                          return (
                            <TableRow
                            ...
                            >
                              ...
                              <TableCell className="text-yellow-500">
                                {formatNumber(
                                  wallet.immediate_transfer_balance
                                )}
                              </TableCell>
                              <TableCell>
                                {wallet.immediate_claim_start
                                  ? new Date(
                                      wallet.immediate_claim_start
                                    ).toLocaleString()
                                  : "N/A"}
                              ...
                              {/* New Cell */}
                              <TableCell>
                                {wallet.claim_interval.toFixed(5)}
                              </TableCell>
                              <TableCell>
                                {wallet.last_claimed
                                  ? new Date(
