...
const formSchema = z.object({
  ...
  renouncementStartTime: z
    .string()
    .min(1, "Renouncement Start Time is required"),
  ...
  immediateClaimStartTime: z
    .string()
    .min(1, "Immediate Claim Start Time is required"), // New field for immediate claim start time
});
...
const Create: React.FC = () => {
  ...
  const ADMIN_CAP_OBJECT_ID = getAdminCapObjectId();
      ...
      const coinsWithNames = await Promise.all(
        balances.map(async (balance) => {
          ...
          if (coinDetails && coinDetails.id) {
            console.log(
              `Fetched decimals for ${coinDetails.name}: ${coinDetails.decimals}`
            );
            const coins = [];
            let result = null;
            let cursor = null;
            ...
            const adjustedBalance =
              Number(balance.totalBalance) / Math.pow(10, coinDetails.decimals);
      ...
      Papa.parse<CsvData>(file, {
        ...
        complete: (results) => {
          ...
          if (validData.length > 0) {
            toast.success(
              `Successfully loaded ${validData.length} valid entries from CSV.`
            );
          } else {
          ...
          if (results.data.length !== validData.length) {
            toast.warn(
              `${results.data.length - validData.length} invalid or duplicate entries were found and skipped.`
            );
          }
        },
        error: (error) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (!ADMIN_CAP_OBJECT_ID) {
      ...
      toast.error(
        "AdminCap not found. Please ensure you have the correct AdminCap object."
      );
      return;
    }
    ...
    if (
      ...
      toast.error(
        "Start date, Renouncement Start, Immediate Claim Start, or Renouncement End is not set"
      );
      return;
    }
    ...
    const startTimeMsBigInt = BigInt(
      convertDateTimeToTimestamp(startDate, startTime)
    );
    const renouncementStartMs = BigInt(
      convertDateTimeToTimestamp(renouncementStart, renouncementStartTime)
    );
    const renouncementEndMs = BigInt(
      convertDateTimeToTimestamp(renouncementEnd, renouncementEndTime)
    );
    const immediateClaimStartMsBigInt = BigInt(
      convertDateTimeToTimestamp(immediateClaimStart, immediateClaimStartTime)
    );
    ...
    const selectedCoinDecimals = coins.find(
      (c) => c.coinObjectId === coin
    )?.decimals;
  ...
  const createSingleVesting = async (
    ...
    const scaledAmount = BigInt(
      Math.floor(amount * Math.pow(10, selectedCoinDecimals))
    );
  ...
  const createBatchVestings = async (
    ...
    toast.success(
      `Batch vesting creation completed for ${csvData.length} entries`
    );
  };

  const convertDateTimeToTimestamp = (date: Date, time: string): number => {
    const [hour, minute, period] = time.split(/[: ]/);
    const hour24 = convertTo24HourFormat(
      parseInt(hour, 10),
      period as "AM" | "PM"
    );
                ...
                >
                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                            ...
                            <Select
                              onValueChange={(value) => {
                                ...
                                const selectedCoinData = coins.find(
                                  (coin) => coin.coinObjectId === value
                                );
                                if (selectedCoinData) {
                                  setSelectedCoinDecimals(
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Select the start date for the vesting.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Enter the start time (e.g., 12:30 PM).
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={() => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Enter the duration of the vesting period.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Select the unit of the claim interval.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Select the start date for renouncement.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Enter the start time for renouncement (e.g., 12:30
                            PM).
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Select the end date for renouncement.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Enter the end time for renouncement (e.g., 12:30
                            PM).
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Select the start date for immediate claim.
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Enter the start time for immediate claim (e.g.,
                            12:30 PM).
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={() => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Enter the immediate transfer percentage (0-100).
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormItem>
                      ...
                      />
                      <FormDescription>
                        Upload a CSV file with columns receiverAddress and
                        amount.
                      </FormDescription>
                    </FormItem>
                    <Button
                    ...
                    <div className="mt-4">
                      <p className=" text-gray-400 text-xs">Digest: {digest}</p>
                    </div>
                  </form>
                </Form>
              </div>
