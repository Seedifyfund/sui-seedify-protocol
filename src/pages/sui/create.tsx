...
import { convertToMilliseconds, convertTo24HourFormat } from '../../lib/calculations';
...
const Create: React.FC = () => {
  ...
  const fetchBalances = async (ownerAddress: string) => {
    try {
      const balances: CoinBalance[] = await client.getAllBalances({ owner: ownerAddress });
      const coinsWithNames = await Promise.all(balances.map(async (balance) => {
        const coinDetails = await client.getCoinMetadata({ coinType: balance.coinType });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    console.log("Setting gas payment with:", { objectId: gasObjectId, version: gasVersion, digest: gasDigest });
    ...
    try {
      ...
      try {
        ...
        const response = await axios.post("/api/Vesting",
          {
            ...data,
  ...
  return (
    <>
      ...
      <FormMessage />
                        ...
                        <><FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormControl>
                                <Select onValueChange={(value) => { field.onChange(value); setSelectedCoin(value); } } defaultValue="">
                                  <SelectTrigger>
                            ...
                            <FormDescription>Enter the start hour (1-12).</FormDescription>
                            <FormMessage />
                          </FormItem></>
                      )}
                    />
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>Select AM or PM.</FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>Enter the start minute (0-59).</FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input type="text" {...field} className="mt-1 block w-full border-gray-600 bg-gray-700 text-white rounded-md shadow-sm" />
                          </FormControl>
                    ...
                    <Button type="submit" className="bg-blue-600 text-white hover:bg-blue-700">Create Vesting</Button>
                    <div className="mt-4">
